...
public class SeckillGoodsPushTask {
    ...
    public void  loadSecKillGoodsToRedis(){
         ...
         * 2.将秒杀商品存入缓存
         */
        // Get range of the seckill time (Maximum : 5 ).
        List<Date> dateMenus = DateUtil.getDateMenus(); // 5个
        System.out.println("Add goods to redis every 30 seconds ");
        for (Date dateMenu : dateMenus) {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

            String redisExtName = DateUtil.date2Str(dateMenu); // Re-trim the data format.
            ...
            //  Which goods are available during this period  (The return "key" value is a set ,it is mean that they are a list )
            Set keys = redisTemplate.boundHashOps(SECKILL_GOODS_KEY + redisExtName).keys();//key field value
            //   Ignore them when they exist in redis.
            if (keys != null && keys.size()>0){
                criteria.andNotIn("id",keys);
            }

            // Get data from mysql database if that data does not exist in redis.
            List<SeckillGoods> seckillGoodsList = seckillGoodsMapper.selectByExample(example);

            //添加到缓存中
            for (SeckillGoods seckillGoods : seckillGoodsList) {
                // Load the details of goods to redis cache.
                redisTemplate.opsForHash().put(SECKILL_GOODS_KEY + redisExtName,seckillGoods.getId(),seckillGoods);

                //加载秒杀商品的库存     One queue for each goods , prevent over-sell
