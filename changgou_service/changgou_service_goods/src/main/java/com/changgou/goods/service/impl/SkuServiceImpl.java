...
//    }
    ...
    // Map<?,?>  集合中是有多个 键值对的 且key 一定要为String类型，  因此需要迭代循环出来  iterate
    @Override
    public void decrCount(Map<String, Integer> decrMap) {
        // 这里容易出现 超卖 现象， 多线程操作   我们使用的 数据库引擎时InnoDB,因此每条sql语句都具有行级锁
        for (Map.Entry<String, Integer> entry : decrMap.entrySet()) {
            ...
            //采用行级锁控制超卖   update tb_sku set num=num-#{num},sale_num=sale_num+#{num} where id=#{skuId} and num>=#{num}
            //为了避免耦合性过高，我们不用 对象执行sql语句的方法
